...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            ...
            self._config_path = (
                self._project_root / "utils/json-validator/config/validator_config.json"
            )
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Config loaded successfully. Keys: {list(self._config.keys())}"
            )
    ...
    def validate_config(self) -> List[str]:
        ...
        if max_errors is not None and (
            not isinstance(max_errors, int) or max_errors < 0
        ):
            errors.append(
                "validation.max_errors_per_object must be a non-negative integer"
            )
    ...
    def reset_to_defaults(self) -> None:
        ...
        """
        default_config = {
            "schema": {"root_path": "schema", "cache_enabled": True},
            "validation": {"strict_mode": True, "max_errors_per_object": 100},
            "output": {"format": "json", "log_level": "INFO"},
