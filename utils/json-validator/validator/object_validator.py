...
class ObjectValidator:
    ...
    def validate_object(self, object_data: Dict[str, Any]) -> ValidationResult:
        ...
        if not object_type_result.is_valid:
            self.logger.warning(
                f"object_type属性の検証に失敗しました: {object_type_result.errors}"
            )
        ...
        if not schema:
            error_msg = (
                f"object_type '{object_type_obj}' に対応するスキーマが見つかりません"
            )
            self.logger.error(error_msg)
            result.add_error(error_msg)
            return result
        ...
        if not validation_result.is_valid:
            self.logger.warning(
                f"JSONSchema検証でエラーが発生しました: {validation_result.errors}"
            )
        ...
        if result.is_valid:
            self.logger.info(
                f"オブジェクト検証が正常に完了しました (object_type: {object_type_obj}, 検証時間: {validation_time:.3f}秒)"
            )
        else:
            self.logger.warning(
                f"オブジェクト検証が失敗しました (object_type: {object_type_obj}, エラー数: {len(result.errors)}, 検証時間: {validation_time:.3f}秒)"
            )
    ...
    def validate_objects(self, objects: List[Dict[str, Any]]) -> ValidationResult:
        ...
        start_time = time.time()
        self.logger.info(
            f"複数オブジェクトの一括検証を開始します (対象件数: {len(objects) if isinstance(objects, list) else 'N/A'})"
        )
        ...
        if result.is_valid:
            self.logger.info(
                f"一括検証が正常に完了しました (成功: {successful_count}, 失敗: {failed_count}, 検証時間: {validation_time:.3f}秒)"
            )
        else:
            self.logger.warning(
                f"一括検証でエラーが発生しました (成功: {successful_count}, 失敗: {failed_count}, 検証時間: {validation_time:.3f}秒)"
            )

        return result

    def validate_object_with_schema(
        self, object_data: Dict[str, Any], schema: Dict[str, Any]
    ) -> ValidationResult:
        """
        指定されたスキーマでオブジェクトを検証
    ...
    def format_validation_error(
        self, error: ValidationError, object_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        検証エラーをフォーマット
    ...
    def _update_validation_stats(
        self, object_type: ObjectType, is_valid: bool, validation_time: float
    ) -> None:
        """
        検証統計情報を更新（内部メソッド）
        ...
        """
        self.logger.debug(
            f"統計情報を更新します (object_type: {object_type}, 結果: {'成功' if is_valid else '失敗'}, 時間: {validation_time:.3f}秒)"
        )
        ...
        self.validation_stats["object_type_counts"][object_type_str] = (
            self.validation_stats["object_type_counts"][object_type_str] + 1
        )
        ...
        self.logger.debug(
            f"統計更新完了 - 総計: {total}, 成功: {success}, 失敗: {failed}, 成功率: {(success/total*100):.1f}%"
            if total > 0
            else "統計更新完了 - 総計: 0"
        )
    ...
    def _get_object_schema(self, object_type: ObjectType) -> Optional[Dict[str, Any]]:
        ...
        if schema:
            self.logger.debug(
                f"スキーマ取得成功: {schema.get('$id', 'N/A')} (object_type: {object_type})"
            )
        else:
            self.logger.warning(
                f"スキーマが見つかりませんでした: object_type '{object_type}'"
            )
    ...
    def _validate_with_jsonschema(
        ...
        try:
            ...
            self.logger.warning(
                f"{error_msg} (パス: {'.'.join(str(x) for x in e.absolute_path) if e.absolute_path else 'root'})"
            )
            result.add_error(error_msg)
