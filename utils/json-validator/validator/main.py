...
class JSONValidator:
    ...
    def validate(self, file_path: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False, errors=[f"予期しないエラー: {str(e)}"]
            )
    ...
    def validate_directory(
        self, directory_path: str, pattern: str = "*.jocf.json"
        ...
        if not directory_path or not directory_path.strip():
            error_result = ValidationResult(
                is_valid=False, errors=["ディレクトリパスが空です"]
            )
            return AggregatedValidationResult([error_result])

        if not pattern or not pattern.strip():
            error_result = ValidationResult(
                is_valid=False, errors=["ファイルパターンが空です"]
            )
            return AggregatedValidationResult([error_result])
        ...
        try:
            ...
            if not directory.is_dir():
                ...
                error_result = ValidationResult(
                    is_valid=False,
                    errors=[f"指定されたパスはディレクトリではありません: {directory}"],
                )
                return AggregatedValidationResult([error_result])
            ...
            file_path_strings = [
                str(file_path) for file_path in file_paths if file_path.is_file()
            ]
            ...
            error_result = ValidationResult(
                is_valid=False,
                errors=[f"ディレクトリアクセス権限がありません: {directory_path}"],
            )
            return AggregatedValidationResult([error_result])
        except Exception as e:
            logger.error(f"Unexpected error during directory validation: {e}")
            error_result = ValidationResult(
                is_valid=False, errors=[f"ディレクトリ検証エラー: {str(e)}"]
            )
            return AggregatedValidationResult([error_result])
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            f"JSONValidator(config_path={self.config_manager.get_config_file_path()})"
        )
