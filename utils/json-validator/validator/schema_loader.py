...
class SchemaLoader:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        self.logger.debug(
            f"SchemaLoader initialized with root path: {self.schema_root_path}"
        )
    ...
    def load_all_schemas(self) -> None:
        ...
        self.logger.debug(
            f"Schema loading completed. Files: {len(self.file_type_map)}, Objects: {len(self.object_type_map)}"
        )
    ...
    def _register_file_schema(self, schema: Dict[str, Any]) -> None:
        ...
        if file_type_str:
            try:
                ...
                self.logger.debug(
                    f"Failed to create FileType from '{file_type_str}': {e}"
                )
        else:
            self.logger.debug(
                f"Could not extract file_type from schema: {schema.get('$id', 'unknown')}"
            )
    ...
    def _register_object_schema(self, schema: Dict[str, Any]) -> None:
        ...
        if object_type_str:
            try:
                ...
                self.logger.debug(
                    f"Failed to create ObjectType from '{object_type_str}': {e}"
                )
        else:
            self.logger.debug(
                f"Could not extract object_type from schema: {schema.get('$id', 'unknown')}"
            )
    ...
    def _build_registry(self) -> Registry:
        ...
        self.logger.debug(
            f"Registry created with {file_schema_count} file schemas and {object_schema_count} object schemas"
        )
        return registry
    ...
    def _extract_file_type(self, schema: Dict[str, Any]) -> Optional[str]:
        ...
        if "file_type" in schema:
            ...
            if "const" in file_type_prop:
                # const値が存在する場合はそれを返す
                return (
                    file_type_prop["const"]
                    if isinstance(file_type_prop["const"], str)
                    else None
                )
            return None
    ...
    def _extract_object_type(self, schema: Dict[str, Any]) -> Optional[str]:
        ...
        if "object_type" in schema:
            ...
            if "const" in object_type_prop:
                # const値が存在する場合はそれを返す
                return (
                    object_type_prop["const"]
                    if isinstance(object_type_prop["const"], str)
                    else None
                )
            return None
    ...
    def _find_schema_files(
        self, directory: Path, pattern: str = "*.schema.json"
    ) -> List[Path]:
        """
    ...
    def _load_additional_schemas_for_registry(self, registry: Registry) -> Registry:
        ...
        for dir_name in additional_dirs:
            ...
            if schema_dir.exists():
                ...
                self.logger.debug(
                    f"Found {len(schema_files)} additional schema files in {dir_name}"
                )
                ...
                for schema_file in schema_files:
                    try:
                        ...
                        if schema_id:
                            ...
                            registry = registry.with_resource(schema_id, resource)
                            self.logger.debug(
                                f"Added additional schema to registry: {schema_id}"
                            )
                    except Exception as e:
                        self.logger.debug(
                            f"Failed to load additional schema {schema_file}: {e}"
                        )
                        continue
            else:
                self.logger.debug(
                    f"Additional schema directory does not exist: {schema_dir}"
                )
