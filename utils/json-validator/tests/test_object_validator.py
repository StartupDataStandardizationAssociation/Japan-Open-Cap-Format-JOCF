...
class TestObjectValidator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.valid_security_holder = {
            ...
            "name": "テスト投資家",
            "contact_info": {"email": "test@example.com", "phone": "090-1234-5678"},
        }

    def test_validate_object_success_stock_issuance(self):
        """正常系: 株式発行オブジェクトの検証成功"""
        # モックの設定
        self.mock_schema_loader.get_object_schema.return_value = (
            self.stock_issuance_schema
        )
        ...
        with patch("jsonschema.validate") as mock_validate:
            ...
            self.mock_schema_loader.get_object_schema.assert_called_once_with(
                expected_call
            )
    ...
    def test_validate_object_unknown_object_type(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertIn(
            "object_type 'UNKNOWN_OBJECT_TYPE' に対応するスキーマが見つかりません",
            result.errors,
        )
    ...
    def test_validate_object_invalid_field_type(self):
        ...
        with patch("jsonschema.validate") as mock_validate:
            mock_validate.side_effect = ValidationError(
                "20230101 is not of type 'string'"
            )
    ...
    def test_validate_object_with_ref_resolution(self):
        ...
        schema_with_ref = {
            ...
            "type": "object",
            "properties": {
                "object_type": {"const": "COMPLEX_OBJECT"},
                "id": {"type": "string"},
                "monetary_value": {
                    "$ref": "https://jocf.startupstandard.org/jocf/main/schema/types/Monetary.schema.json"
                },
        ...
        test_data = {
            "object_type": "COMPLEX_OBJECT",
            "id": "test-complex-1",
            "monetary_value": {"amount": "1000", "currency_code": "JPY"},
        }
        ...
        with patch("jsonschema.validate", wraps=jsonschema.validate) as mock_validate:
            ...
            mock_validate.assert_called_once_with(
                test_data, schema_with_ref, registry=registry
            )
    ...
    def test_validate_object_nested_ref_resolution_error(self):
        ...
        schema_with_invalid_ref = {
            ...
            "type": "object",
            "properties": {
                "object_type": {"const": "INVALID_REF_OBJECT"},
                "id": {"type": "string"},
                "invalid_ref": {
    ...
    def test_validate_complex_nested_object(self):
        ...
        complex_object = {
            ...
            "share_price": {"amount": "5000", "currency_code": "JPY"},
            "quantity": "10000",
            "date": "2023-06-15",
            ...
            "nested_data": {
                "agreement_details": {
                    "clauses": [
                        {"type": "anti_dilution", "description": "希薄化防止条項"},
                        {
    ...
    def test_get_supported_object_types(self):
        ...
        self.mock_schema_loader.get_object_types.return_value = [
            "TX_STOCK_ISSUANCE",
            "SECURITY_HOLDER",
            "TX_STOCK_TRANSFER",
        ]
    ...
    def test_get_validation_context(self):
        """正常系: 検証コンテキスト取得"""
        context = self.object_validator.get_validation_context(
            self.valid_stock_issuance
        )
    ...
    def test_format_validation_error(self):
        ...
        formatted = self.object_validator.format_validation_error(
            error, self.valid_stock_issuance
        )
    ...
    def test_get_schema_for_object_success(self):
        ...
        object_type = self.object_validator.get_object_type(self.valid_stock_issuance)
        schema = (
            self.object_validator._get_object_schema(object_type)
            if object_type
            else None
        )
    ...
    def test_update_validation_stats(self):
        ...
        self.assertEqual(
            updated_stats["total_validations"], initial_stats["total_validations"] + 1
        )
        self.assertEqual(
            updated_stats["successful_validations"],
            initial_stats["successful_validations"] + 1,
        )
...
class TestContactInfoValidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.security_holder_contact_info_schema = {
            ...
            "additionalProperties": False,
            "required": ["email"],  # ❌ 問題: 'email'が必須だが実際は'emails'配列
        }
    ...
    def test_contact_info_required_field_mismatch(self):
        ...
        contact_info_data = {
            "phone_numbers": [
                {"phone_number": "03-1234-5678", "phone_type": "BUSINESS"}
            ],
        ...
        with patch("jsonschema.validate") as mock_validate:
            ...
            self.assertTrue(
                any(
                    "'email' is a required property" in error for error in result.errors
                )
            )
