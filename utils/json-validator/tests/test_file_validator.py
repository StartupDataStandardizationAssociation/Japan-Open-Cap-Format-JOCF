...
class MockFileValidator:
    ...
    def validate_file(self, file_data):
        ...
        if not schema:
            result.add_error(
                f"file_type '{file_type}' に対応するスキーマが見つかりません"
            )
            return result
...
class TestFileValidator(unittest.TestCase):
    ...
    def test_validate_file_unknown_file_type(self):
        ...
        self.assertFalse(result.is_valid)
        self.assertIn(
            "file_type 'UNKNOWN_FILE_TYPE' に対応するスキーマが見つかりません",
            result.errors,
        )
    ...
    def test_validate_items_array_invalid_object_type(self):
        ...
        self.assertFalse(
            result.is_valid, "無効なobject_typeが含まれているため検証失敗するべき"
        )

        # object_type検証エラーまたはオブジェクト検証エラーが含まれているべき
        error_messages = str(result.errors)
        has_object_type_error = (
            "INVALID_OBJECT_TYPE" in error_messages
            or "オブジェクト検証エラー" in error_messages
        )
        self.assertTrue(
            has_object_type_error,
            f"無効なobject_typeに関するエラーが含まれるべき: {result.errors}",
        )
    ...
    def test_validate_file_type_property_validation(self):
        ...
        self.assertTrue(
            self.file_validator._validate_file_type(invalid_data_empty)
        )  # 空文字列は文字列として有効
    ...
    def test_complex_file_validation(self):
        ...
        complex_file_data = {
            ...
            "additional_info": {"version": "1.0", "created_at": "2023-01-01T00:00:00Z"},
        }
    ...
    def test_mixed_object_types_in_items(self):
        ...
        mixed_items_data = {
            "file_type": "JOCF_TRANSACTIONS_FILE",
            "items": [
                {"object_type": "TX_STOCK_ISSUANCE", "id": "issuance-1"},
                {"object_type": "TX_STOCK_TRANSFER", "id": "transfer-1"},
                {"object_type": "TX_CONVERTIBLE_ISSUANCE", "id": "convertible-1"},
                {"object_type": "TX_STOCK_CONVERSION", "id": "conversion-1"},
            ],
        }
        ...
        self.assertFalse(
            result.is_valid, "必須フィールドが不足しているため検証失敗が期待される"
        )
        self.assertGreater(
            len(result.errors), 0, "オブジェクト検証エラーが発生するべき"
        )
                ...
                if result.is_valid and self.object_validator:
                    ...
                    for item in items:
                        ...
                        if not obj_result.is_valid:
                            result.add_error(
                                f"オブジェクト検証失敗: {obj_result.errors}"
                            )
    ...
    def test_requirement_3_items_type_check_invalid_object_type(self):
        ...
        self.assertFalse(
            result.is_valid,
            "許可されていないobject_typeが含まれているため検証は失敗するべき",
        )
        self.assertIn(
            "UNAUTHORIZED_TYPE",
            str(result.errors),
            "許可されていないobject_typeに関するエラーメッセージが含まれるべき",
        )
    ...
    def test_requirement_4_items_object_validation(self):
        ...
        if not item_1_result.is_valid or not item_2_result.is_valid:
            ...
            self.assertFalse(
                result.is_valid,
                "ObjectValidatorでitems要素の検証が失敗している場合、FileValidator全体も失敗するべき",
            )

            # ObjectValidatorのエラーがFileValidatorのエラーに含まれているべき
            has_object_validation_error = any(
                "オブジェクト検証エラー" in str(error) for error in result.errors
            )
            self.assertTrue(
                has_object_validation_error,
                "FileValidatorのエラーにオブジェクト検証エラーが含まれるべき",
            )

            print(
                f"✅ items配列の各要素がObjectValidatorで検証され、エラーが正しく検出されました"
            )
        else:
            # ObjectValidatorが成功している場合、このテストは適切でない
            self.fail(
                "テストデータが不適切です。ObjectValidatorで失敗するデータを使用してください"
            )
    ...
    def test_requirement_5_other_attributes_with_object_type(self):
        ...
        # スキーマ設定を適用
        self.mock_schema_loader.get_file_schema.return_value = (
            schema_with_other_attributes
        )
        ...
        if "issuer_info" in valid_items_data:
            issuer_result = self.file_validator.object_validator.validate_object(
                valid_items_data["issuer_info"]
            )
            print(
                f"Manual issuer_info validation: {issuer_result.is_valid}, errors: {issuer_result.errors}"
            )
            ...
            if not issuer_result.is_valid:
                ...
                self.assertFalse(
                    result.is_valid,
                    "issuer_infoのオブジェクト検証が失敗している場合、ファイル検証全体も失敗するべき",
                )

                # issuer_infoに関する詳細エラーメッセージが含まれているべき
                has_issuer_error = any(
                    "issuer_info" in str(error) for error in result.errors
                )
                self.assertTrue(
                    has_issuer_error,
                    f"issuer_infoのオブジェクト検証エラーがFileValidatorで検出されるべき: {result.errors}",
                )

                print(
                    f"✅ issuer_infoのオブジェクト検証エラーがFileValidatorで適切に検出されました"
                )
            else:
    ...
    def test_requirement_6_detailed_error_messages(self):
        ...
        self.assertGreater(
            len(result.errors), 0, "詳細エラーメッセージが出力されるべき"
        )
        ...
        if hasattr(result, "get_summary"):
            ...
            self.assertGreater(
                categories.get("object_validation_errors", 0),
                0,
                "オブジェクト検証エラーが分類されるべき",
            )
            self.assertGreater(
                categories.get("type_check_errors", 0),
                0,
                "型チェックエラーが分類されるべき",
            )
            ...
            # エラーサマリー機能が未実装の場合
            self.fail(
                "ValidationResult.get_summary()メソッドが存在しません。"
                "エラーサマリー機能が実装されるべきです。"
            )
    ...
    def test_requirement_integration_full_file_validation_flow(self):
        ...
        self.assertFalse(
            result.is_valid,
            "items内のオブジェクトに必須フィールドが不足している可能性があるため",
        )
...
class TestGetAllowedObjectTypes(unittest.TestCase):
    ...
    def test_get_allowed_object_types_with_direct_ref_structure(self):
        ...
        resolved_schema = {
            ...
            "properties": {"object_type": {"const": "SECURITY_HOLDER"}},
        }
        ...
        self.assertEqual(
            result, ["SECURITY_HOLDER"], "直接$ref構造からobject_typeが取得できるべき"
        )
    ...
    def test_get_allowed_object_types_with_oneOf_structure(self):
        ...
        stock_transfer_schema = {
            "properties": {"object_type": {"const": "TX_STOCK_TRANSFER"}}
        }
        ...
        self.assertEqual(
            set(result),
            {"TX_STOCK_ISSUANCE", "TX_STOCK_TRANSFER"},
            "oneOf構造から全てのobject_typeが取得できるべき",
        )
...
class TestNameTypeValidation(unittest.TestCase):
    ...
    def test_name_field_should_be_object_not_string(self):
        ...
        validation_result.add_error(
            "JSONスキーマ検証エラー: '創業者 A(移転元)' is not of type 'object'"
        )
        mock_object_validator.validate_object.return_value = validation_result

        # テスト実行
        result = self.file_validator.object_validator.validate_object(
            security_holder_data, "SECURITY_HOLDER"
        )

        # 検証（現在は文字列形式でエラーになる）
        self.assertFalse(result.is_valid, "文字列形式のnameはエラーになるべき")
        self.assertTrue(
            any("is not of type 'object'" in error for error in result.errors),
            "Name型の型エラーが発生するべき",
        )
    ...
    def test_name_field_correct_object_format(self):
        ...
        security_holder_data = {
            "object_type": "SECURITY_HOLDER",
            "id": "test-securityholder-investor-a",
            "name": {"legal_name": "創業者 A(移転元)"},  # 正しい: オブジェクト形式
            "security_holder_type": "INDIVIDUAL",
        }
    ...
    def test_numeric_field_should_be_string_not_number(self):
        ...
        stock_class_data = {
            ...
            "preffered_stock_attributes": {
                "liquidation_preference_attributes": {"seniority": 3}  # 問題: 数値形式
            },
        }
        ...
        # テスト実行
        result = self.file_validator.object_validator.validate_object(
            stock_class_data, "STOCK_CLASS"
        )

        # 検証（現在は数値形式でエラーになる）
        self.assertFalse(result.is_valid, "数値形式のseniorityはエラーになるべき")
        self.assertTrue(
            any("is not of type 'string'" in error for error in result.errors),
...
class TestConversionMechanismValidation(unittest.TestCase):
    ...
    def test_ratio_field_should_be_object_not_string(self):
        ...
        # テスト実行
        result = self.file_validator.object_validator.validate_object(
            conversion_mechanism_data, "RATIO_CONVERSION"
        )
        ...
        self.assertTrue(
            any(
                "not valid under any of the given schemas" in error
    ...
    def test_ratio_field_correct_object_format(self):
        ...
        conversion_mechanism_data = {
            ...
            "description": "転換価額が調整された同数のA種優先株への転換(払込不要)",
            "conversion_price": {"amount": "0", "currency": "JPY"},
            "ratio": {"numerator": "1", "denominator": "1"},  # 正しい: オブジェクト形式
            "rounding_type": "FLOOR",
        }
        ...
        # 検証（オブジェクト形式は成功するべき）
        self.assertTrue(
            result.is_valid, "正しいオブジェクト形式のratioは検証に通るべき"
        )
