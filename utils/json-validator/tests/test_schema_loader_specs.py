...
class TestSchemaLoaderBasicUsageSpecs(unittest.TestCase):
    ...
    def test_typical_usage_workflow_spec(self):
        ...
        self.assertGreater(
            len(loader.file_type_map), 0, "ファイルスキーマがロードされている"
        )
        self.assertGreater(
            len(loader.object_type_map), 0, "オブジェクトスキーマがロードされている"
        )

        self.assertIsNotNone(
            transactions_schema, "TRANSACTIONSファイルスキーマが取得できる"
        )
        self.assertEqual(transactions_schema.get("title"), "トランザクション")

        self.assertIsNotNone(
            stock_issuance_schema, "株式発行オブジェクトスキーマが取得できる"
        )
...
class TestSchemaLoaderWorkflowSpecs(unittest.TestCase):
    ...
    def test_load_all_schemas_workflow_spec(self):
        ...
        self.assertGreater(
            len(self.loader.file_type_map), 0, "ファイルスキーマがロードされる"
        )
        self.assertGreater(
            len(self.loader.object_type_map), 0, "オブジェクトスキーマがロードされる"
        )
        ...
        for file_type_str in expected_file_types:
            ...
            self.assertIn(
                file_type, self.loader.file_type_map, f"{file_type_str}がロードされる"
            )

        # And: 期待されるobject_typeが含まれる
        expected_object_types = ["TX_STOCK_ISSUANCE", "SECURITY_HOLDER", "STOCK_CLASS"]
        for object_type_str in expected_object_types:
            object_type = ObjectType(object_type_str)
            self.assertIn(
                object_type,
                self.loader.object_type_map,
                f"{object_type_str}がロードされる",
            )
    ...
    def test_multiple_load_all_schemas_calls_spec(self):
        ...
        # Then: 結果は同じである（冪等性）
        self.assertEqual(
            len(self.loader.file_type_map),
            initial_file_count,
            "複数回呼び出しても結果は同じ",
        )
        self.assertEqual(
            len(self.loader.object_type_map),
...
class TestSchemaLoaderRegistryIntegrationSpecs(unittest.TestCase):
    ...
    def test_registry_integration_with_jsonschema_validation(self):
        ...
        # And: $refを含むスキーマを取得
        stock_issuance_schema = self.loader.get_object_schema(
        ...
        self.assertTrue(
            validation_success, "Registryを使った$ref解決付き検証が成功する"
        )
...
class TestSchemaLoaderDataIntegritySpecs(unittest.TestCase):
    ...
    def test_schema_uniqueness_spec(self):
        ...
        self.assertEqual(
            len(file_types), len(set(file_types)), "file_typeキーは一意である"
        )

        # And: object_type_mapでのキーの一意性
        object_types = list(self.loader.object_type_map.keys())
        self.assertEqual(
            len(object_types), len(set(object_types)), "object_typeキーは一意である"
        )

        # And: file_typeとobject_typeに重複がない
        type_intersection = set(file_types) & set(object_types)
        self.assertEqual(
            len(type_intersection), 0, "file_typeとobject_typeに重複はない"
        )
...
class TestSchemaLoaderAdditionalMethodsSpecs(unittest.TestCase):
    ...
    def test_get_schema_by_id_usage_spec(self):
        ...
        self.assertIn(
            "title", transactions_schema, "取得したスキーマは完全な内容を持つ"
        )
        self.assertIn(
            "properties", transactions_schema, "取得したスキーマは完全な内容を持つ"
        )
    ...
    def test_get_types_methods_usage_spec(self):
        ...
        self.assertGreater(
            len(combined_types["file_types"]), 0, "ドキュメント用データ構造を構築"
        )
        self.assertGreater(
            len(combined_types["object_types"]), 0, "ドキュメント用データ構造を構築"
        )
    ...
    def test_schema_metadata_access_pattern_spec(self):
        ...
        self.assertGreater(
            summary_info["total_object_schemas"], 0, "オブジェクトスキーマ数"
        )

        # 管理画面での表示用データとして利用可能
        total_schemas = (
            summary_info["total_file_schemas"] + summary_info["total_object_schemas"]
        )
    ...
    def test_cache_management_usage_spec(self):
        ...
        initial_count = len(fresh_loader.file_type_map) + len(
            fresh_loader.object_type_map
        )
    ...
    def test_string_representation_usage_spec(self):
        ...
        self.assertTrue(
            status_display["is_loaded"], "管理画面用ステータス情報として使用可能"
        )
        self.assertIsInstance(
            status_display["readable_status"], str, "表示用文字列が取得可能"
        )
