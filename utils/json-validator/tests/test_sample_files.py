...
class TestRealWorldFiles(unittest.TestCase):
    ...
    def test_validate_case_files(self):
        ...
        self.assertEqual(
            invalid_files,
            0,
            f"ケースファイルでバリデーションエラーが発生しています: {invalid_files}個のファイルでエラー",
        )
        self.assertEqual(
            valid_files,
            len(case_files),
            "すべてのケースファイルが有効である必要があります",
        )
    ...
    def test_validate_j_kiss_files(self):
        ...
        self.assertEqual(
            invalid_files,
            0,
            f"J-KISSファイルでバリデーションエラーが発生しています: {invalid_files}個のファイルでエラー",
        )
        self.assertEqual(
            valid_files,
            len(j_kiss_files),
            "すべてのJ-KISSファイルが有効である必要があります",
        )
    ...
    def test_validate_seeds_files(self):
        ...
        network_errors = [
            ...
            if "HTTPSConnectionPool" in error or "Failed to resolve" in error
        ]
        self.assertEqual(
            len(network_errors),
            0,
            "ネットワークエラーが発生しています - スキーマの$ref解決に問題があります",
        )
        ...
        self.assertEqual(
            invalid_files,
            0,
            f"SEEDSファイルでバリデーションエラーが発生しています: {invalid_files}個のファイルでエラー",
        )
        self.assertEqual(
            valid_files,
            len(seeds_files),
            "すべてのSEEDSファイルが有効である必要があります",
        )
    ...
    def test_validate_stock_repurchase_files(self):
        ...
        self.assertGreater(
            len(stock_repurchase_files),
            0,
            "Stock repurchaseファイルが見つかりませんでした",
        )
        ...
        self.assertEqual(
            invalid_files,
            0,
            f"Stock repurchaseファイルでバリデーションエラーが発生しています: {invalid_files}個のファイルでエラー",
        )
        self.assertEqual(
            valid_files,
            len(stock_repurchase_files),
            "すべてのStock repurchaseファイルが有効である必要があります",
        )
    ...
    def test_validate_stocktransfer_files(self):
        ...
        self.assertGreater(
            len(stocktransfer_files), 0, "Stock transferファイルが見つかりませんでした"
        )
        ...
        self.assertEqual(
            invalid_files,
            0,
            f"Stock transferファイルでバリデーションエラーが発生しています: {invalid_files}個のファイルでエラー",
        )
        self.assertEqual(
            valid_files,
            len(stocktransfer_files),
            "すべてのStock transferファイルが有効である必要があります",
        )
    ...
    def test_validate_all_sample_directories(self):
        ...
        for dir_name, display_name in sample_directories:
            ...
            if not sample_dir.exists():
                print(
                    f"{display_name}ディレクトリ ({dir_name}) が存在しません - スキップ"
                )
                continue
                    ...
                    if subdir.is_dir():
                        for numbered_dir in subdir.iterdir():
                            if numbered_dir.is_dir():
                                ...
                                files.extend(
                                    [numbered_dir]
                                    if numbered_dir.suffix == ".json"
                                    and "jocf" in numbered_dir.name
                                    else []
                                )
            else:
                # 直接JOCFファイルを持つディレクトリ
            ...
            print(
                f"{display_name}: {len(files)}ファイル (有効: {valid_files}, 無効: {invalid_files})"
            )

        print(f"{'='*60}")
        print(
            f"合計: {total_files}ファイル (有効: {total_valid}, 無効: {total_invalid})"
        )
        print(
            f"成功率: {(total_valid/total_files*100):.1f}%"
            if total_files > 0
            else "成功率: N/A"
        )
        print(f"{'='*60}")
        ...
        self.assertGreater(
            total_files, 0, "サンプルファイルが1つも見つかりませんでした"
        )
        self.assertEqual(
            total_invalid,
            0,
            f"サンプルファイルでバリデーションエラーが発生しています: {total_invalid}個のファイルでエラー",
        )
        self.assertEqual(
            total_valid,
            total_files,
            "すべてのサンプルファイルが有効である必要があります",
        )
    ...
    def test_config_based_sample_directory(self):
        ...
        # 環境変数での上書きテスト
        with patch.dict(
            os.environ, {"VALIDATOR_TESTING_SAMPLES_DIR": "custom_samples"}
        ):
