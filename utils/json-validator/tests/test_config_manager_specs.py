...
from validator.config_manager import ConfigManager, get_config_manager, reset_config_manager, load_config_from_dict
    ...
    def test_path_config_access(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            try:
                ...
                assert schema_root.name == "custom_schema"  # ディレクトリ名が正しいことを確認
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            try:
                ...
                with patch.dict('os.environ', {
                    ...
                    assert cm.get("validation.strict_mode") is True  # 文字列"true"がbooleanに変換
                    ...
                    assert 'OTHER_VARIABLE' not in env_overrides  # プレフィックスなしは除外
    ...
    def test_config_merging_and_reset(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            try:
                ...
                # 設定のマージ
                merge_config = {
                    "schema": {"cache_enabled": False, "auto_reload": True},  # 既存を上書き + 新規追加
