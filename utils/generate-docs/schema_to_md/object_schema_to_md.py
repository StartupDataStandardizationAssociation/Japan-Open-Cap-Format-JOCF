        ...
        for prop_name, prop_data in schema['properties'].items():
            ...
            if prop_type == "object":
                ...
                for sub_prop_name, sub_prop_data in prop_data['properties'].items():
                    sub_prop_type = get_property_type(sub_prop_data, input_file_relative_path_to_root)
                    # 子プロパティ情報の出力
                    ...
                    md_lines.append(f"| {prop_name}.{sub_prop_name} | {sub_prop_type} | {sub_required} | {sub_description} |")
                    if sub_prop_type == "object":
                        # さらにオブジェクト型の場合、孫スキーマを再帰的に処理
                        for sub_sub_prop_name, sub_sub_prop_data in sub_prop_data['properties'].items():
                            sub_sub_prop_type = get_property_type(sub_sub_prop_data, input_file_relative_path_to_root)
                            sub_sub_required = is_required_property(sub_sub_prop_name, prop_data)
                            sub_sub_description = get_property_description(sub_sub_prop_data, prop_data)
                            # 孫プロパティ情報の出力
                            md_lines.append(f"| {prop_name}.{sub_prop_name}.{sub_sub_prop_name} | {sub_sub_prop_type} | {sub_sub_required} | {sub_sub_description} |")
            if prop_type == "array of object":
                # 配列の要素がオブジェクト型の場合、子スキーマを再帰的に処理
                for sub_prop_name, sub_prop_data in prop_data['items']['properties'].items():
                    sub_prop_type = get_property_type(sub_prop_data, input_file_relative_path_to_root)
                    sub_required = is_required_property(sub_prop_name, prop_data)
                    sub_description = get_property_description(sub_prop_data, prop_data)
                    md_lines.append(f"| {prop_name}.{sub_prop_name} | {sub_prop_type} | {sub_required} | {sub_description} |")
